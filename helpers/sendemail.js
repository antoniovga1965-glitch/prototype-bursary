...
router.post('/resetpassword',async(req,res)=>{
    ...
    try {
        const existing = await prisma.registered_users.findUnique({
            where:{REGISTEREMAIL:RESETPASSWORD},
        })
        if(!existing){
...
router.post('/savenewpassword',async(req,res)=>{
    ...
    try {
         const user = await prisma.registered_users.findFirst({
        where:{
            token,
            resetTokenExpiry:
            {gt:new Date()}
